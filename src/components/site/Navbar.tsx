...
export default function Navbar() {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/10 bg-slate-950/75 backdrop-blur-xl">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-6">
        ...
        <div className="hidden shrink-0 items-center gap-2 xl:flex">
          <a
            ...
            className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2.5 text-sm font-bold text-cyan-100 transition hover:bg-cyan-300 hover:text-slate-950"
          >
            Iniciar sesión
          </a>
          <a
            href={NEURO_REGISTER_URL}
            className="rounded-full bg-cyan-300 px-4 py-2.5 text-sm font-black text-slate-950 transition hover:bg-white"
          >
            Probar 30 días
          </a>
        </div>
        ...
        <button
          ...
          className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white xl:hidden"
      ...
      {open ? (
        <div className="fixed inset-x-0 top-20 z-40 border-b border-white/10 bg-slate-950/95 px-6 py-6 shadow-2xl backdrop-blur-xl xl:hidden">
